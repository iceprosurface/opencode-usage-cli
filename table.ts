...
export class ResponsiveTable {
	...
	/**
	 * Gets current table head and col aligns based on compact mode
	...
	/**
	 * Returns whether table is currently in compact mode
	...
	/**
	 * Renders table as a formatted string
	...
	toString(): string {
		...
		// Check if this fits in terminal
	 ...
	 * @returns True if row is a separator
	 */
 ...
 * @param extraColumns - Number of extra empty columns before data (default: 1 for models column)
 * @param trailingColumns - Number of extra empty columns after data (default: 0)
 */
export function pushBreakdownRows(
	...
	for (const breakdown of breakdowns) {
		const row: (string | number)[] = firstColumnEmpty
			? ['']
			: [`  └─ ${formatModelName(breakdown.modelName)}`];

		if (firstColumnEmpty) {
			row.push(`  └─ ${formatModelName(breakdown.modelName)}`);
		}
 ...
 * @param config - Configuration options for table
...
/**
 * Formats a usage data row for display in table
 * @param firstColumnValue - Value for first column (date, month, etc.)
...
/**
 * Adds an empty separator row to table for visual separation
